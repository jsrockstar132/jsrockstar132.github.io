/** Detect Mobile Browsers from : http://detectmobilebrowsers.com/about **/
...
window.onload = function(){
    ...
    $('.primary').css('min-height', $('.secondary').height());
    
    $('.primary').on('mouseenter', function(){
    	if(!$('.secondary').hasClass('opacity30'))
	    	$('.secondary').addClass('opacity30');
    });
    
    $('.primary').on('mouseleave', function(){
    	if($('.secondary').hasClass('opacity30'))
	    	$('.secondary').removeClass('opacity30');
    });
};
